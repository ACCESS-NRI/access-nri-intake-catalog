...
def test_access_time_parser(times, bounds, ffreq, expected):
    if bounds:
        time = (times[0] + times[1]) / 2
        ds = xr.Dataset(
            data_vars={
...
def test_generic_time_parser(times, bounds, ffreq, expected):
    if bounds:
        time = (times[0] + times[1]) / 2
        ds = xr.Dataset(
            data_vars={
...
def test_gfdl_time_parser(times, ffreq, expected):
    ...
    results = GfdlTimeParser(ds, filename_frequency=ffreq, time_dim="time")()
